...
class BirdBushTests: XCTestCase {
    ...
    override func setUp() {
        ...
        bigIndex = BirdBush(locations: bigArray, nodeSize: 64, getID: { return Int($0[0]) }, getX: { return $0[1] }, getY: { return $0[2] })
    ...
    func testBuild() {
        XCTAssertEqual(index?.coords, self.coords)
        XCTAssertEqual(index?.ids, self.ids)
    }
    ...
    func testRandomPointNN() {
        for _ in 1...10000 {
            ...
            XCTAssertEqual(nn, bruteNN(qx: randX, qy: randY, index: bigIndex!))
        }
    }
    
    func bruteNN(qx: Double, qy: Double, index: BirdBush) -> Int {
